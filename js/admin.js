...
function fetchOrders() {
    ...
    function showEditOrderModal(order) {
        ...
        const contactInfo = order.order_user_contact || ''; 
        ...
        orderInfoParts.forEach((part) => {
            const imagePattern = /(ORD\S+\.jpg|ToppingImage-\S+\.(jpg|png)|SuggestionImage-\S+\.(jpg|png))/gi;
            ...
            if (imageMatches.length > 0) {
                ...
                imageMatches.forEach((match) => {
                    ...
                    if (imageName.startsWith('ToppingImage-')) {
                        imagePath = 'php/uploads/ToppingImage/' + imageName;
                        orderInfoHTML += `<br><a href="#" class="image-link" data-image-path="${imagePath}" data-image-name="${imageName}">Image: ${imageName}</a>`;
                    } else if (imageName.startsWith('SuggestionImage-')) {
                        imagePath = 'php/uploads/SuggestionImage/' + imageName;
                        orderInfoHTML += `<br><a href="#" class="image-link" data-image-path="${imagePath}" data-image-name="${imageName}">Image: ${imageName}</a>`;
                    } else if (imageName.startsWith('ORD') && imageName.endsWith('.jpg')) {
                        imagePath = 'php/uploads/3DCustom/' + imageName;
                        orderInfoHTML += `<br><img src="${imagePath}" alt="${imageName}" class="displayed-image" style="max-width: 100%; height: auto;">`;
                    }
                });
            } else {
                // Append text parts
...
function populateEditForm(product) {
    // Populate the product ID and name
    ...
    // Handle "No Size" and "Have Size" logic with price handling
    const sizeRadioButtons = document.getElementsByName('size_option'); // Get all radio buttons for size options

    if (product.cake_no_size === 1) {
        // "No Size" selected
        document.getElementById('noSizeRadio').checked = true;
        document.getElementById('editNoSizeInputContainer').style.display = 'block';  // Show the no-size price input
        document.getElementById('editSizeCheckboxes').style.display = 'none';  // Hide the size checkboxes
        document.getElementById('editNoSizePrice').value = product.cake_price_small || ''; // Set the price for 'No Size'
        
        // Hide all size inputs
        document.getElementById('editCostSmallContainer').style.display = 'none';
        document.getElementById('editCostMediumContainer').style.display = 'none';
        document.getElementById('editCostLargeContainer').style.display = 'none';
    } else if (product.cake_no_size === 0) {
        // "Have Size" selected
        document.getElementById('haveSizeRadio').checked = true;
        document.getElementById('editNoSizeInputContainer').style.display = 'none';  // Hide the no-size price input
        document.getElementById('editSizeCheckboxes').style.display = 'block';  // Show the size checkboxes
        
        // Handle small size
        if (product.cake_price_small !== 0) {
            document.getElementById('editSizeSmall').checked = true;
            document.getElementById('editCostSmallContainer').style.display = 'block';
            document.getElementById('editCostSmall').value = product.cake_price_small;
        } else {
            document.getElementById('editSizeSmall').checked = false;
            document.getElementById('editCostSmallContainer').style.display = 'none';
            document.getElementById('editCostSmall').value = ''; // Clear the value if the price is 0
        }

        // Handle medium size
        if (product.cake_price_medium !== 0) {
            document.getElementById('editSizeMedium').checked = true;
            document.getElementById('editCostMediumContainer').style.display = 'block';
            document.getElementById('editCostMedium').value = product.cake_price_medium;
        } else {
            document.getElementById('editSizeMedium').checked = false;
            document.getElementById('editCostMediumContainer').style.display = 'none';
            document.getElementById('editCostMedium').value = ''; // Clear the value if the price is 0
        }

        // Handle large size
        if (product.cake_price_large !== 0) {
            document.getElementById('editSizeLarge').checked = true;
            document.getElementById('editCostLargeContainer').style.display = 'block';
            document.getElementById('editCostLarge').value = product.cake_price_large;
        } else {
            document.getElementById('editSizeLarge').checked = false;
            document.getElementById('editCostLargeContainer').style.display = 'none';
            document.getElementById('editCostLarge').value = ''; // Clear the value if the price is 0
        }
    }
    ...
    categoryDropdown.value = product.cake_category; // Set category value
...
function confirmEdit() {
    ...
    // Handle No Size logic
    if (document.getElementById('noSizeRadio').checked) {
        // Only pass the No Size price (cake_price_small)
        formData.set('cake_no_size', 1);  // Set No Size flag
        formData.set('cake_price_small', document.getElementById('editNoSizePrice').value || null); // Set No Size price
        formData.set('cake_price_medium', null); // Do not pass medium price
        formData.set('cake_price_large', null); // Do not pass large price
    } else if (document.getElementById('haveSizeRadio').checked) {
        // "Have Size" selected
        formData.set('cake_no_size', 0);  // Set Have Size flag
        formData.set('cake_price_small', document.getElementById('editSizeSmall').checked ? document.getElementById('editCostSmall').value : null);
        formData.set('cake_price_medium', document.getElementById('editSizeMedium').checked ? document.getElementById('editCostMedium').value : null);
        formData.set('cake_price_large', document.getElementById('editSizeLarge').checked ? document.getElementById('editCostLarge').value : null);
    }
    ...
    function fetchOrders(statusFilter = '') {
        ...
        fetch(url)
            ...
            .then(orders => {
                ...
                // Clear previous rows
                while (orderTableBody.rows.length > 0) {
...
function filterOrders(status) {
    ...
    fetch(`php/fetch_orders_complete.php`)
        ...
        .then(orders => {
            ...
            orderTableComplete.innerHTML = ""; // Clear existing rows

            // Iterate through orders and add them to the table
            orders.forEach(order => {
                ...
                // Add click event to show the invoice modal
                row.addEventListener('click', () => {
                    showInvoiceModal(order.order_no); // Replace with your function to show invoice modal
                });
        ...
        let tableRows = reviews.length === 0
            ...
            : reviews.map(review => `
                <tr data-order-no="${review.order_no}" data-star-rating="${review.order_rating}">
                    ...
                    <td>
                        <div class="d-flex align-items-center">
                            <div class="form-check form-switch" style="margin-top: 0; margin-bottom: 0; vertical-align: middle; text-align: center;">
                                <input 
...
function generateIncomeReport() {
    ...
    .then(data => {
        if (data.success) {
            ...
            alert('Error fetching report data: ' + (data.message || 'Unknown error'));
        }
    })
    .catch(error => {
        console.error('Error:', error);  // Catching and logging any errors that occur
